...
export default function Donation() {
  ...
  const reportMutation = trpc.content.submitBtcTransfer.useMutation({ onSuccess: () => { setPaid(true); setTxHash(""); toast.success("USDC payment submitted for review."); } });
  ...
  const submitPaymentReport = (event: React.FormEvent) => { event.preventDefault(); const emailForReport = donorEmail.trim() || user?.email?.trim() || ""; if (!txHash.trim() || !emailForReport) { toast.error("Enter the transaction hash and donor email."); return; } reportMutation.mutate({ currencyCode: "USDC", txHash, amount: selectedAmount, email: emailForReport }); };
