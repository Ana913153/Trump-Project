...
export async function createBtcTransfer(values: { userId?: number | null; childId?: number | null; email?: string | null; txHash: string; amount?: string | null; currencyCode?: string }): Promise<BtcTransfer | undefined> { const db = await getDb(); if (!db) throw new Error("Database is not configured"); const result = await db.insert(btcTransfers).values({ ...values, currencyCode: values.currencyCode || "BTC" }); return db.select().from(btcTransfers).where(eq(btcTransfers.id, Number(result[0].insertId))).limit(1).then((rows) => rows[0]); }
export async function listBtcTransfers() { const db = await getDb(); if (!db) return []; return db.select({ id: btcTransfers.id, userId: btcTransfers.userId, childId: btcTransfers.childId, email: btcTransfers.email, txHash: btcTransfers.txHash, amount: btcTransfers.amount, currencyCode: btcTransfers.currencyCode, status: btcTransfers.status, note: btcTransfers.note, createdAt: btcTransfers.createdAt, updatedAt: btcTransfers.updatedAt, donorName: users.name, donorUsername: users.username }).from(btcTransfers).leftJoin(users, eq(btcTransfers.userId, users.id)).orderBy(desc(btcTransfers.createdAt)); }
