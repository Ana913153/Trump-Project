...
function normalizeNewYorkScheduleDate(date: Date) { const parts = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(date); const get = (type: string) => Number(parts.find((part) => part.type === type)?.value); return new Date(Date.UTC(get("year"), get("month") - 1, get("day"), 12, 0, 0)); }
...
export const appRouter = router({
  ...
  admin: router({
    ...
    setContributionPlan: adminProcedure.input(z.object({ childId: z.number().int().positive(), monthlyAmountCents: cents, nextContributionAt: z.coerce.date() })).mutation(async ({ input }) => { const child = await getChildById(input.childId); if (!child) throw new TRPCError({ code: "NOT_FOUND", message: "Child account not found." }); return createContributionPlan({ ...input, nextContributionAt: normalizeNewYorkScheduleDate(input.nextContributionAt) }); }),
    ...
    createFooterLink: adminProcedure.input(z.object({ title: z.string().trim().min(2).max(120), url: z.string().trim().min(1).max(500).refine((value) => value.startsWith("/") || /^https?:\/\//.test(value), "Invalid footer link"), body: z.string().trim().min(2).max(10000) })).mutation(({ input }) => createFooterLink(input)),
    deleteFooterLink: adminProcedure.input(z.object({ id: z.number().int().positive() })).mutation(({ input }) => deleteFooterLink(input.id)),
