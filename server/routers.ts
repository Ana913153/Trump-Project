...
export const appRouter = router({
  ...
  admin: router({
    ...
    createFooterLink: adminProcedure.input(z.object({ title: z.string().trim().min(2).max(120), url: z.string().trim().min(1).max(500).refine((value) => value.startsWith("/") || /^https?:\/\//.test(value), "Invalid footer link") })).mutation(({ input }) => createFooterLink(input)),
